# Summary

[소개](README.md)

# 프로그래밍 언어

- [Rust](./rust/README.md)
    - [기초 문법](./rust/syntax-related/README.md)
    - [기본 개념](./rust/basic-concepts/README.md)
        - [메모리 안전성을 위한 기본 규칙 - Ownership, Borrowing, Ailiasing, Lifetime의 이해](./rust/basic-concepts/231228-memory-safety-rule-basic.md)
        - [메모리 안전성을 위한 심화 규칙 - Partial move, Partial borrow의 이해](./rust/basic-concepts/231228-memory-safety-rule-advanced.md)
        - [메모리 안전성을 위한 심화 규칙 - Reborrowing의 이해](./rust/basic-concepts/240122-memory-safety-rule-reborrow.md)
        - [Shadowing의 이해와 활용](./rust/basic-concepts/231222-variable-shadowing.md)
    - [기능](./rust/features/README.md)
    - [컴파일 에러](./rust/compile-error/README.md)
        - [반환 위치에서의 impl Trait VS Trait bounds](./rust/compile-error/231219-impl-vs-generics-at-return-position.md)
        - [Generic 매개변수의 lifetime 처리](./rust/compile-error/231220-generic-argument-lifetime.md)
    - [팁](./rust/tips/README.md)
        - [함수의 선언부가 가지는 의미](./rust/tips/231220-meaning-of-function-signature.md)
    - [기타](./rust/uncategorized/README.md)

# 알고리즘

- [알고리즘](./algorithms/README.md)
